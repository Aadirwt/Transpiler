...
class InputHandler:
    ...
    def preprocess(self, code: str) -> PreprocessedInput:
        ...
        for raw_line in normalized.splitlines():
            line = raw_line.strip()
            import_match = re.fullmatch(r"import\s+([A-Za-z_][A-Za-z0-9_]*)(?:\s+as\s+([A-Za-z_][A-Za-z0-9_]*))?", line)
            ...
            if import_match:
                ...
                if import_match.group(2):
                    warnings.append(
                        f"Import alias '{import_match.group(2)}' is recorded but not fully modeled."
                    )
                continue
        ...
        return PreprocessedInput(code="\n".join(kept_lines).strip(), imports=imports, warnings=warnings)
